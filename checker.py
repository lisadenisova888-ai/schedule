"""Проверка готового расписания на конфликты ресурсов.

Проверка не пересобирает расписание и не пытается исправлять занятия. Она
приводит каждую запись к удобному внутреннему виду и ищет пересечения:

1. ``norm_lesson`` переводит строку времени в минуты, разбивает поле группы
   на отдельные группы и собирает нормализованные поля занятия.
2. ``validator`` раскладывает занятия по трём индексам занятости:
   ``group_busy``, ``teacher_busy`` и ``auditoriums_busy``. Благодаря этому
   сравниваются только занятия, которые используют один и тот же ресурс.
3. Для каждого ресурса занятия сортируются по дню и времени. Затем пары
   занятий одного дня проверяются формулой пересечения полуинтервалов:
   ``start1 < end2`` и ``start2 < end1``.
4. При пересечении формируется человекочитаемая ошибка с ресурсом, двумя
   занятиями и фактическим общим интервалом времени.
5. Некорректная запись времени или обязательных полей не останавливает
   проверку всего файла: она превращается в отдельную ошибку занятия.

Список ``unplaced_lessons`` не считается конфликтом. CLI выводит его отдельно,
чтобы показать занятия, которые планировщик не смог разместить.
"""
...
def validator(schedule_data):
    """Вернуть список конфликтов групп, преподавателей и аудиторий."""
    errors=[]
