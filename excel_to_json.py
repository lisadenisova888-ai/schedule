"""Конвертер Excel-файлов проекта в JSON.

Модуль поддерживает два входных сценария.

Исходные данные для генерации расписания:

1. Имена листов нормализуются без учёта регистра и варианта ``ё/е``.
   Русские и английские названия листов сводятся к разделам ``аудитории``,
   ``преподаватели``, ``предметы`` и ``группы``.
2. Первая строка каждого листа считается заголовком. Пустые строки
   пропускаются, а номер Excel-строки сохраняется для точных сообщений об
   ошибках.
3. Для каждого раздела используются допустимые алиасы колонок. Числовые поля
   валидируются, списки id и условий разбираются из значений через запятую или
   точку с запятой, тип занятия приводится к коду лекции или семинара.
4. После конвертации получается JSON той структуры, которую напрямую читает
   ``scheduler.schedule``.

Готовое расписание:

1. Если листов исходных данных нет, модуль ищет лист ``Все занятия``,
   ``schedule`` или ``lessons``.
2. Строки этого листа переводятся в элементы ``lessons`` с полями дня,
   времени, группы, предмета, типа, преподавателя и аудитории.
3. Для такого файла возвращается уже готовое расписание, а
   ``unplaced_lessons`` создаётся пустым.

Кроме чтения Excel модуль умеет создать шаблон книги: записывает обязательные
листы, заголовки, стили заголовков и фиксирует строку заголовка.
"""
...
def convert_teachers(rows):
    ...
    for row in rows:
        ...
        teachers.append({
            ...
            "преподает предметы (id)": parse_list(
...
def convert_source_workbook(workbook):
    ...
    return {
        ...
        "преподаватели": convert_teachers(sheet_rows(sheets["преподаватели"])),
        "предметы": convert_subjects(sheet_rows(sheets["предметы"])),
        "группы": convert_groups(sheet_rows(sheets["группы"])),
    }
...
def convert_excel_to_json(input_file, output_file):
    """Определить формат Excel-книги, конвертировать её и записать JSON."""
