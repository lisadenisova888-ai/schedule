"""Построение учебного расписания из исходных данных.

Алгоритм работает как жадный планировщик с оценкой всех допустимых вариантов:

1. Из входного JSON берутся аудитории, преподаватели, предметы и группы.
   Для предметов строится индекс по ``id``, чтобы быстро находить описание
   предмета по ссылкам из групп и преподавателей.
2. ``build_lesson_tasks`` разворачивает недельную нагрузку в отдельные задачи
   размещения. Количество занятий вычисляется из часов предмета, длительности
   занятия и длительности академического часа. Лекции одного предмета
   объединяют все группы в одну задачу, семинары создаются отдельно для каждой
   группы.
3. Задачи сортируются так, чтобы сначала ставить длинные и более нагруженные
   занятия: для них позже сложнее найти свободный интервал.
4. Для каждой задачи перебираются дни и старты с шагом ``braek_min`` в пределах
   учебного дня. Вариант отбрасывается, если хотя бы одна группа занята, нет
   свободного преподавателя с нужным предметом и остатком нагрузки или нет
   свободной аудитории нужной вместимости.
5. Каждый допустимый вариант получает score. В нём сильнее всего штрафуются
   неподходящие аудитории, затем окна у групп и преподавателя; более слабые
   штрафы предпочитают раннее время и выравнивают нагрузку преподавателей.
6. Лучший вариант фиксируется в таблицах занятости групп, преподавателей и
   аудиторий. Если вариантов нет, задача попадает в ``unplaced_lessons`` с
   общей причиной невозможности размещения.

Результат содержит отсортированный список ``lessons`` и список занятий,
которые алгоритм не смог поставить без нарушения ограничений.
"""
...
def build_lesson_tasks(groups, subj_id):
    """Развернуть учебную нагрузку групп в задачи для жадного планировщика."""
    tasks=[]
...
def schedule(data):
    """Найти лучший доступный слот для каждой задачи и собрать расписание."""
    aud=data["аудитории"]
