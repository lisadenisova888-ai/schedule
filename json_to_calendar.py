"""Конвертер готового JSON-расписания в календарь iCalendar (.ics).

Алгоритм экспорта превращает каждое занятие в отдельный ``VEVENT``:

1. Из входного JSON читается список ``lessons``. Дата начала учебной недели
   берётся из параметра ``week_start`` или выбирается как ближайший понедельник.
2. Название дня недели превращается в смещение от понедельника, а диапазон
   времени превращается в ``datetime`` начала и конца события. Если конец
   оказался раньше начала, событие считается переходящим через полночь.
3. Из предмета, типа, группы, преподавателя и аудитории формируются поля
   ``SUMMARY``, ``DESCRIPTION`` и ``LOCATION``. Текст экранируется по правилам
   iCalendar.
4. UID события вычисляется как SHA-1 от стабильных полей занятия и даты
   события. Это даёт одинаковый идентификатор при повторном экспорте одного
   занятия на ту же учебную неделю.
5. При ``repeat_weeks > 1`` в событие добавляется недельное правило ``RRULE``.
6. Все строки календаря складываются в ``VCALENDAR`` и переносятся по границе
   75 UTF-8 байт, как требует формат ICS.

Параметр timezone записывается в ``DTSTART``, ``DTEND`` и метаданные календаря,
поэтому импортирующий календарь получает локальное время занятий.
"""
...
def convert_json_to_calendar(
    ...
    """Собрать ICS-календарь из списка занятий и вернуть статистику экспорта."""
